...
function constraints(filePath) {
    ...
    traverse(result, function (node) {
        ...
        if (node.type === 'FunctionDeclaration') {
            ...
            traverse(node, function(child) {
                ...
                if(_.get(child, 'type') === 'BinaryExpression' && _.includes(['!=', '!==', '==', '===', '='], _.get(child, 'operator'))) {
                    ...
                    // Handle else if for not an identifier
                    else if(_.get(child, 'left.type') ===  'CallExpression') {
                        var leftChild = child.left;
			var ident = leftChild.callee.object.name;
                        ...
                        if(leftChild.callee && leftChild.callee.property.name == "indexOf") {
                            var str = "\"" +  leftChild.arguments[0].value + "\"";
                            let constraints = functionConstraints[funcName].constraints[ident];
                            constraints.push(new Constraint({
		...
		if (_.get(child, 'type') === 'UnaryExpression' && _.includes(['!'], _.get(child, 'operator'))) {
    			if (_.get(child, 'argument.type') === 'Identifier' && _.includes(params, _.get(child, 'argument.name'))) {
    			...
    			else if (_.get(child, 'argument.type') === 'MemberExpression' && _.includes(params, _.get(child, 'argument.object.name'))) {
